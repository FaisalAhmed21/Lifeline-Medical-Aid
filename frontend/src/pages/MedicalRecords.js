    ...
    try {
      ...
      if (fetchedFiles.length > 0 && fetchedFiles[0].recordId) {
        try {
          ...
          // Helper to format diagnosis into a string to avoid [object Object]
          const formatDiagnosis = (diag) => {
            ...
            if (typeof diag === 'object') {
              if (diag.primary) return String(diag.primary);
              try {
                const vals = Object.values(diag).map(v => {
          ...
          setFormData({
            ...
            diagnosis: formatDiagnosis(fullRecord.diagnosis?.primary || fullRecord.diagnosis || ''),
